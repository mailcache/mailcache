...
Jim is the MailCache Chaos Monkey, inspired by Netflix.

You can invite Jim to the party using the `invite-jim` flag:

    MailCache -invite-jim
...
of it happening to 0, e.g. to disable connection rate limiting:
    ...
    MailCache -invite-jim -jim-linkspeed-affect=0

### Examples

Always rate limit to 1 byte per second:

    MailCache -invite-jim -jim-linkspeed-affect=1 -jim-linkspeed-max=1 -jim-linkspeed-min=1

Disconnect clients after approximately 5 commands:

    MailCache -invite-jim -jim-disconnect=0.2

Simulate a mobile connection (at 10-100kbps) for 10% of clients:

    MailCache -invite-jim -jim-linkspeed-affect=0.1 -jim-linkspeed-min=1250 -jim-linkspeed-max=12500
